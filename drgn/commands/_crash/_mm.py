...
from drgn import Architecture, FaultError, Object, ObjectNotFoundError, Platform, Program
...
def _get_vm_flag_values(prog: Program) -> Sequence[Tuple[str, int]]:
    ...
    for name, bit_num in enumerators:
        flags.append(
            (
                name.removeprefix("VMA_").removesuffix("_BIT"),
                1 << bit_num,
            )
        )
    return flags
        ...
        if args.m:
            ...
            if mm:
                ...
                for vma in for_each_vma(mm):
                    ...
                    translation_rows: List[Sequence[Any]] = [
                        ("VIRTUAL", "PHYSICAL")
                    ]
